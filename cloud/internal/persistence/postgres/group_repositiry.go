...
func NewGroupRepository(db *database.DBProvider) identity.GroupRepository {
	return &groupRepository{db: db}
}

// CreateGroup creates a new group in the database.
func (r *groupRepository) CreateGroup(ctx context.Context, group *model.Group) error {
	return r.db.Get().WithContext(ctx).Create(group).Error
}
...
func (r *groupRepository) AddUserToGroup(ctx context.Context, tenantID, userID, groupID uuid.UUID) error {
	...
	var group model.Group
	if err := r.db.Get().WithContext(ctx).Where("uuid = ? AND tenant_id = ?", groupID, tenantID).First(&group).Error; err != nil {
		return err // Group not found
	}

	var user model.User
	if err := r.db.Get().WithContext(ctx).Where("uuid = ? AND tenant_id = ?", userID, tenantID).First(&user).Error; err != nil {
		return err // User not found
	}

	// Use GORM's Association to append the user to the group's Users.
	// GORM will handle the `user_groups` join table insertion.
	return r.db.Get().WithContext(ctx).Model(&group).Association("Users").Append(&user)
}
...
func (r *groupRepository) RemoveUserFromGroup(ctx context.Context, tenantID, userID, groupID uuid.UUID) error {
	...
	return r.db.Get().WithContext(ctx).Model(&group).Association("Users").Delete(&user)
}
